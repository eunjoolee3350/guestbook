...
pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', url:'https://github.com/eunjoolee3350/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivacyKey']) {
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: '0lrlokr@gmail.com')
        ...
        success { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'good'
                , message: "lej - ${JOB_NAME} (${BUILD_NUMBER}) 빌드가 성공적으로 끝났습니다. Details: (<${BUILD_URL} | here >)")
        }
        failure { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'danger'
                , message: "lej - ${JOB_NAME} (${BUILD_NUMBER}) 빌드가 실패하였습니다. Details: (<${BUILD_URL} | here >)")
    }
  }
}
